...
public class NormalMovement : CharacterState
{
    ...
    public override void CheckExitTransition()
    {
        ...
        {
            CharacterStateController.EnqueueTransition<PlayerDashAbility>();
        }
    ...
    // float initialLocalVerticalVelocityComponent = 0f;
    ...
    protected virtual void ProcessPlanarMovement( float dt )
    {
        ...
        switch( CharacterActor.CurrentState )
        {
            ...
            case CharacterActorState.StableGrounded:
                ...
                targetPlanarVelocity = CustomUtilities.Multiply( CharacterStateController.InputMovementReference , currentPlanarSpeedLimit , speedMultiplier );
    ...
    // Why? Because the character controller itself (CharacterActor) could eventually modify velocity, rotation and/or size later. If that's the case, we need to send 
    // the updated data to the Animator component.
    ...
    {          
        ...
        // The PostSimulationUpdate (CharacterActor) could update velocity once more (e.g. if a "bad step" is detected).
